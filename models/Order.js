...
exports.getOrderByorder_id = (order_id, callback) => {
  pool.query("SELECT * FROM Order WHERE order_id = ?", [order_id], callback);
};

exports.addOrder = (Order, callback) => {
  const { order_id, order_date, customer_id, total_amount } = Order;
  const query = `INSERT INTO Order (order_date, customer_id, total_amount) VALUES (?, ?, ?)`;
  const values = [order_date, customer_id, total_amount];
...
exports.updateOrder = (Order, callback) => {
  ...
  const values = [order_date, customer_id, total_amount, order_id];
  pool.query(query, values, callback);
};

exports.deleteOrder = (order_id, callback) => {
  pool.query("DELETE FROM Order WHERE order_id = ?", [order_id], callback);
};
